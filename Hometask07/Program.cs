...
double[,] CreateRandomTwoDemArray(int a, int b, int min, int max)
{
    ...
    for(int i = 0; i < a; i++)
    {
        for (int j = 0; j < b; j++)
        {
            newMatrix[i,j] = Math.Round(new Random().Next(7, 7) + new Random().NextDouble(),1);
...
double[,] array = CreateRandomTwoDemArray(7, 7, 1, 9);
...
void CheckElement (int[,] newMatrix, int m, int n)
{
    int result = 0;
    ...
    else Console.Write("This number is absent");

}

CheckElement (newMatrix, m, n);
